...
func doServeAdmitFunc(w http.ResponseWriter, r *http.Request, admit admitFunc) ([]byte, error) {
	...
	// todo: based on the documentation of Decode, it may be safer to provide defualts and use the returned object, rather than &admissionReviewReq
	...
	if err != nil {
		// We still allow the object, only report an error. By not setting the JSON patches, we leave the object untouched
		admissionReviewResponse.Response.Allowed = true
		// todo: log error in admit
	} else {
		// encode the patch operations to JSON and set at the response.
